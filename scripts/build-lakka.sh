...
build_drivers() {
    ...
    # Build OSD binary (32-bit + 64-bit) — requires rpi-userland
    echo "[2/5] Building OSD binary..."
    local osd_dir="$PROJECT_DIR/rpi/osd"
    if [[ ! -d "$osd_dir" ]]; then
        die "OSD source not found at $osd_dir"
    fi
    local userland_dir="/tmp/rpi-userland"
    if [[ ! -d "$userland_dir" ]]; then
        echo "  Cloning rpi-userland..."
        git clone --depth 1 https://github.com/raspberrypi/userland.git "$userland_dir"
    fi
    (
        cd "$osd_dir"
        make 32 || die "Failed to build 32-bit OSD"
        make 64 || die "Failed to build 64-bit OSD"
    )
    cp "$osd_dir/bin/32bit/osd" "$DRIVERS_BIN_DIR/osd_32"
    cp "$osd_dir/bin/64bit/osd" "$DRIVERS_BIN_DIR/osd_64"
    # Copy 32-bit vendor libraries for OSD
    local osd_vendor_32="$osd_dir/vendor/32bit"
    for lib in "$osd_vendor_32"/*; do
        local libname
        libname="$(basename "$lib")"
        cp "$lib" "$DRIVERS_BIN_DIR/$libname"
    ...
    # Rename libasound.so to libasound.so.2 for 32-bit OSD
    ...
    # Copy 64-bit vendor libraries for OSD
    local osd_vendor_64="$osd_dir/vendor/64bit"
    for lib in "$osd_vendor_64"/*; do
        local libname
        libname="$(basename "$lib")"
        cp "$lib" "$DRIVERS_BIN_DIR/$libname"
        # Fix SONAME to match the filename so the dynamic linker resolves it
...
joysticks=1
# Disable On Screen Display (Battery & Volume Display overlay)
enable_osd=true
EOF
...
[[ "$enable_osd" == "true" ]] && /flash/drivers/bin/osd_64
wait
BOOTEOF
    ...
    # Fix RUNPATH on OSD binaries and all vendor libs so transitive dependencies
    # (e.g. libvchiq_arm.so.0, libvcos.so.0 pulled in by libbcm_host.so.0) are
    # resolved from /flash/drivers/bin rather than the system library paths.
    echo "  Fixing OSD RUNPATH..."
    local rpath_dir="${DRIVER_BINARIES_DIR:-$DRIVERS_BIN_DIR}"
    for bin in "$rpath_dir"/osd_32 "$rpath_dir"/osd_64 "$rpath_dir"/lib*.so*; do
        if [[ -f "$bin" ]]; then
            patchelf --set-rpath '/flash/drivers/bin' "$bin" || echo "patchelf failed on $bin"
        fi
    done
...
[Unit]
Description=PSPi gamepad and OSD
